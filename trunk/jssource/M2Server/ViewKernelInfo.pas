...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, ComCtrls, Grids, SDK, grobal2;
...
begin
  try
    ...
    MainOutMessage('[Exception] TfrmViewKernelInfo.FormCreate');
  end;
end;
...
begin
  try
    ...
    MainOutMessage('[Exception] TfrmViewKernelInfo.Open');
  end;
end;
...
begin
  ...
  //  GridThread.Row:=2;
    ...
    MainOutMessage('[Exception] TfrmViewKernelInfo.TimerTimer');
  end;
end;
...
begin
  try
    ...
    GridThread.Cells[3, Index + 1] := format('%d/%d/%d', [GetTickCount -
      ThreadInfo.dwRunTick,
      ThreadInfo.nRunTime,
        ThreadInfo.nMaxRunTime]);
    GridThread.Cells[4, Index + 1] := format('%d', [ThreadInfo.nRunFlag]);
  except
    MainOutMessage('[Exception] TfrmViewKernelInfo.GridThreadAdd');
  end;
end;
