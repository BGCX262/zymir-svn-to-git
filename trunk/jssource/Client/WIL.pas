...
type
  ...
  pTBmpImage = ^TBmpImage;
   ...
   TBmpImageArr  = array[0..MaxListSize div 1024{4}] of TBmpImage;
   TDxImageArr   = array[0..MaxListSize div 1024{4}] of TDxImage;
...
Try
   ...
   {if g_boUseDIBSurface and m_bt32Bit and (not bo32bit) then begin //DIB
      //·ÇÈ«ÆÁÊ±
      try
      lsDib.Clear;
      ...
      //·ÇÈ«ÆÁÊ±
      //imginfo.nWidth:=_MIN(imginfo.nWidth,255);
      //imgInfo.nHeight:=_MIN(imgInfo.nHeight,255);
...
{$IF WINDOFILL   = OPENFILL}
            if g_boUseDIBSurface then begin
              DWBits := PDWORD(integer(ddsd.lpSurface) + (imginfo.nHeight - 1 - n) * ddsd.lPitch);
              for i:=0 to sLen -1 do begin
                DWBits^:=pfRGBEx(SBits^);
                Inc(DWBits);
                Inc(SBits);
              end;
            end else begin
...
{$IFEND}
   ...
   //end;
Except
  //DebugOutStr('[Exception] TWMImages.LoadDxImage ' + Format('%d %d',[slen,imginfo.nHeight])); //³ÌÐò×Ô¶¯Ôö¼Ó
end;
end;
