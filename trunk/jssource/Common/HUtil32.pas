...
uses
	Classes,Windows, Math, SysUtils, StrUtils, WinTypes, WinProcs, Graphics,
  Messages, Dialogs, TlHelp32,WinInet,WinSvc;
...
function  TagCountEx(source: string; tag,tag2: char): integer;
{*}
...
//procedure KillSoft (sName: string);
...
function  Str_ToInt (Str: string; def: Longint): LongWord;
...
function GetCPUIDText(boShow:Boolean):String;
...
begin
   ...
   for i:=1 to Length(source) do
      if (source[i] = tag) or (source[i] = tag2) then Inc (tcount);
   Result := tcount;
end;
  ...
  try
    ...
    try
      repeat
        ...
        if not InternetReadFile(hURL,@Buffer,SizeOf(Buffer)-1,BufferLen) then break;
...
var
  ...
  boTrue:Boolean;
begin
  FillChar(PROCESSENTRY32,SizeOf(tagPROCESSENTRY32),#0);
  PROCESSENTRY32.dwSize:=SizeOf(tagPROCESSENTRY32);
  ProcessHandle:=CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS,0);
  Try
  if ProcessHandle>0 then begin
    boTrue:=Process32First(ProcessHandle,PROCESSENTRY32);
    while boTrue do begin
      if CompareText(sName,PROCESSENTRY32.szExeFile) = 0 then begin
        PHandle:=OpenProcess(1,False,PROCESSENTRY32.th32ProcessID);
        TerminateProcess(PHandle,0);
        break;
      end;
      boTrue:=Process32Next(ProcessHandle,PROCESSENTRY32);
    end;
  end;
  Finally
...
begin
  ...
  if boShow then begin
    Result:='';
    sTemp:=IntToHex(CPUID[1],8);
    Result:=Copy(sTemp,1,4);
    Result:=Result+'-'+Copy(sTemp,5,4);
    sTemp:=IntToHex(CPUID[3],8);
    Result:=Result+'-'+Copy(sTemp,1,4);
    Result:=Result+'-'+Copy(sTemp,5,4);
    sTemp:=IntToHex(CPUID[4],8);
    Result:=Result+'-'+Copy(sTemp,1,4);
    Result:=Result+'-'+Copy(sTemp,5,4);
    //Result:=IntToHex(CPUID[1],8)+'-'+IntToHex(CPUID[3],8)+'-'+IntToHex(CPUID[4],8)
  end else
    Result:=IntToHex(CPUID[1]+CPUID[3]+CPUID[4],0);
end;
