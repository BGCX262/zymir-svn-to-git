...
uses
  Windows, Classes, SysUtils, Grobal2, HUtil32, DXDraws, 
  MShare, Share;
...
//    procedure SaveMapArr(nCurrX, nCurrY: Integer);
  public
    ...
    function  LoadMapInfoEx (): Boolean;
...
begin
  ...
  m_sFileName := m_sMapBase + m_sCurrentMap + '.map';
  if FileExists(m_sFileName) then begin
    nHandle := FileOpen(m_sFileName, fmOpenRead or fmShareDenyNone);
    if nHandle > 0 then begin
      FileRead(nHandle, Header, sizeof(TMapHeader));
...
function  TMap.LoadMapInfoEx (): Boolean;
var
  sFileName    :String;
  nHandle      :Integer;
  Header       :TMapHeader;
...
Except //³ÌÐò×Ô¶¯Ôö¼Ó
  DebugOutStr('[Exception] TMap.LoadMapInfoEx'); //³ÌÐò×Ô¶¯Ôö¼Ó
End; //³ÌÐò×Ô¶¯Ôö¼Ó
end;
...
procedure DrawMiniMap;
var
  sx, sy, ex, ey, i, j, imgnum, wunit, ani, nY, mx, my: Integer;
